...
import itertools
from mtgrn_data import MTGRN_data, MTGRN_test_data
...
def preprocess_three_data(args,logger):
    gene_set = list()
    
    for dataset in ["mHSC-GM","mHSC-L","mHSC-E"]:
        pval_cutoff = 0.001
        select_variable_genes = 2000

        #加载表达矩阵数据
        expr_df = pd.read_csv(f"/home/pengrui/work_space_pengrui/project/GRN期刊/BEELINE-data/inputs/scRNA-Seq/{dataset}/ExpressionData.csv", header=0, index_col=0)
        expr_df.index = expr_df.index.map(lambda x: x.upper())  #把基因名称大写

        # 加载gene差异表达的信息
        different_gene = pd.read_csv(f"/home/pengrui/work_space_pengrui/project/GRN期刊/BEELINE-data/inputs/scRNA-Seq/{dataset}/GeneOrdering.csv",header=0,index_col=0)
        different_gene.index = different_gene.index.map(lambda x: x.upper())
        
        #保证different_gene中的基因都在基因表达矩阵中
        ...
        if select_variable_genes > len(different_gene):    #如果剩下的基因都不足要挑选的基因个数,那么直接赋值即可
            end_variable_genes = different_gene.index.tolist()
        else:
            #如果基因的个数高于要挑选的基因数量,则这些基因根据方差进行排序,然后方差越大的基因(越具有特异性)
        ...
        gene_set = gene_set + end_variable_genes
    whole_gene = sorted(set(gene_set))
    print(f'总共gene数:{len(whole_gene)}')
    pd.DataFrame({'gene':whole_gene}).to_csv(args.output_dir + '/whole_gene.csv')
...
def preprocess_data(args,logger):
    ...
    end_variable_genes = pd.read_csv(args.output_dir + '/whole_gene.csv')['gene'].tolist()

    expr_df = pd.read_csv(args.data_path, header=0, index_col=0)
    expr_df.index = expr_df.index.map(lambda x: x.upper())  #把基因名称大写

    #将基因表达矩阵进行过滤,只挑选上述基因的表达值
    ...
    # 根据ground truth中涉及的node对基因表达矩阵进行过滤
...
def preprocess_data_for_mtgrn(args):
    ...
    expr_df = expr_df.set_index('Unnamed: 0').loc[cell_index_sorted_by_time]        #将基因表达矩阵中的细胞按照时间进行排序
...
def parse_args():
    ...
    parser.add_argument('--gene_order',type=str, help='差异基因排名情况')
    ...
    parser.add_argument('--index',type=int,default=-1,help='用于寻找超参,-1表明非寻找超参阶段')
    
    #model
    parser.add_argument('--cuda_index',type=int,help='使用哪张显卡')
...
def test(args,save_path,checkpoint_path,expression_data,id_gene_df,size,prior_mask,no_tf_genes_index,logFC_value,prior_grn):
    print("\ntesting...")
    test_data_set = MTGRN_test_data(expression_data,size)
    ...
    with torch.no_grad():
        ...
        gene_coefficient = pd.DataFrame(mean_attention,index=id_gene_df['geneName'],columns=id_gene_df['geneName'])
        gene_coefficient.to_csv(os.path.join(save_path,'gene_coefficient.csv'))
...
def main():
    ...
    if args.baseline:
        ...
        preprocess_three_data(args,logger)
        ...
        # 构建训练数据集
        train_data_set = MTGRN_data(expression_data,dropout_mask,size,'train')
        ...
        #构建验证集
        val_data_set = MTGRN_data(expression_data,dropout_mask,size,'val')
