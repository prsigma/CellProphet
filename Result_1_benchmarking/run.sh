...
output_dir="./Result_1_benchmarking/output"
...
for dataset in "${datasets[@]}"; do
    ...
    --gene_order BEELINE-data/inputs/scRNA-Seq/${dataset}/GeneOrdering.csv --tf_path ${tf_path} --species ${species} --logFC_path "BEELINE-data/inputs/scRNA-Seq/${dataset}/DEgenes_MAST_sp4_PseudoTime.csv" --cuda_index 0 --baseline
    else
    /home/pengrui/mambaforge/envs/grn/bin/python main.py --data_source BEELINE --dataset_name ${dataset} --data_path "BEELINE-data/inputs/scRNA-Seq/${dataset}/ExpressionData.csv" \
    --output_dir ${output_dir} --gt_path "BEELINE-Networks/Networks/${species}/${dataset}-ChIP-seq-network.csv" --time_info "BEELINE-data/inputs/scRNA-Seq/${dataset}/PseudoTime.csv" \
    --gene_order BEELINE-data/inputs/scRNA-Seq/${dataset}/GeneOrdering.csv --tf_path ${tf_path} --species ${species} --logFC_path "BEELINE-data/inputs/scRNA-Seq/${dataset}/DEgenes_MAST_sp4_PseudoTime.csv" --cuda_index 0 --baseline
    fi
done